...
export default function MultiMusicSelect() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-black via-gray-900 to-blue-900 gap-6">
      ...
      <h2 className="text-3xl font-bold mb-6 text-white drop-shadow">マルチプレイ楽曲選択</h2>
      
      {!showCustom && (
        <div className="flex flex-col gap-4 w-full max-w-xs">
          <h3 className="text-xl font-semibold text-white text-center mb-2">チュートリアル楽曲</h3>
          {difficulties.map((d, i) => (
            <button
              key={d}
              className={`px-6 py-4 rounded-xl text-xl font-bold shadow-lg transition text-white ${
                i === 0 ? 'bg-green-500 hover:bg-green-600' : 
                i === 1 ? 'bg-yellow-500 hover:bg-yellow-600' : 
      ...
      {showCustom && (
        <div className="flex flex-col items-center gap-4 w-full max-w-md">
          ...
          <h3 className="text-2xl font-semibold mb-2 text-white">カスタム楽曲リスト</h3>
          
          {loading && <div className="text-white">読み込み中...</div>}
          {!loading && customCharts.length === 0 && <div className="text-white">楽曲がありません</div>}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full max-h-96 overflow-y-auto">
            {customCharts.map(chart => (
              ...
              >
                ...
                <span className="text-sm text-blue-100">BPM: {chart.bpm || 120}</span>
                <span className="text-xs text-blue-200">作成者: {chart.createdBy}</span>
              </button>
            ))}
          </div>
        </div>
