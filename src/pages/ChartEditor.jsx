...
const LANE_COLORS = [
  {
    ...
    note: 'bg-orange-300',
    noteBorder: 'border-orange-500',
  },
  {
    bg: 'bg-pink-900/50',
    border: 'border-pink-400',
    note: 'bg-pink-300',
    noteBorder: 'border-pink-500',
  },
  {
    bg: 'bg-cyan-900/50',
    border: 'border-cyan-400',
    note: 'bg-cyan-300',
    noteBorder: 'border-cyan-500',
  },
  {
    bg: 'bg-green-900/50',
    border: 'border-green-400',
    note: 'bg-green-300',
    noteBorder: 'border-green-500',
  },
]

const EditorLane = React.memo(
  React.forwardRef(
    (
      {
        lane,
...
export default function ChartEditor() {
  ...
  const fetchCharts = useCallback(async () => {
    try {
      ...
      setAllCharts(
        allChartsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }))
      )

      if (currentUser) {
        const userChartsQuery = query(
          collection(db, 'charts'),
          where('createdBy', '==', currentUser.uid)
        )
        const userChartsSnapshot = await getDocs(userChartsQuery)
        setUserCharts(
          userChartsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }))
        )
      } else {
        setUserCharts([])
      }
  ...
  const handleAudioError = () => {
    setAudioError(true)
    console.error('音声ファイルの読み込みに失敗:', selectedSong)
  }
    ...
    time => {
      if (laneRefs.current.length > 0 && laneRefs.current[0]) {
        ...
        const targetScrollLeft =
          (time / duration) * GRID_WIDTH - visibleWidth / 2
  ...
  const saveChart = async () => {
    ...
    if (notes.length === 0 && !confirm('ノーツがありませんが保存しますか？'))
      return
    ...
    const chartData = {
      ...
      audioTitle:
        ...
        LOCAL_SONGS.find(s => s.url === selectedSong)?.title ||
        '音源',
      bpm: Number(bpm),
        ...
        <div className="bg-white/10 rounded-2xl shadow-2xl p-6 w-full max-w-4xl">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`px-3 py-1 rounded text-sm ${!showAllCharts ? 'bg-blue-500' : 'bg-gray-600'}`}
              >
                自分の譜面
              </button>
              <button
          ...
          <div className="max-h-96 overflow-y-auto space-y-2">
            {(showAllCharts ? allCharts : userCharts).map(chart => (
              <div
                key={chart.id}
                className="bg-gray-800/50 hover:bg-gray-700/50 rounded p-3 flex justify-between items-center"
              >
                <button
                ...
                >
                  ...
                  <div className="text-sm text-gray-400">
                    BPM: {chart.bpm} | 長さ: {Math.round(chart.duration)}秒 |
                    音源: {chart.audioTitle}
                  </div>
                </button>
                {chart.createdBy === currentUser?.uid && (
                  <button
                    ...
                    className="ml-4 px-3 py-1 bg-red-600 hover:bg-red-700 text-white text-xs rounded"
                  >
                    削除
                  </button>
                )}
          ...
          <div className="flex gap-2 mt-4">
            ...
            <button
              onClick={() => setIsChartListVisible(false)}
          ...
          <div className="bg-white/10 rounded-2xl shadow-2xl p-6 flex flex-col gap-4 lg:w-1/3">
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 px-4 py-2 bg-blue-500 rounded"
              >
                新規作成
              </button>
              <button
                onClick={() => setIsChartListVisible(true)}
                className="flex-1 px-4 py-2 bg-purple-500 rounded"
              >
                譜面一覧
              </button>
            </div>
            <div>
              ...
              <input
                ...
                className="w-full p-2 mt-1 rounded bg-gray-700 focus:ring-2 focus:ring-yellow-400"
              />
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <input
                ...
                className="text-sm file:mr-2 file:py-1 file:px-2 file:rounded file:border-0 file:bg-blue-600 hover:file:bg-blue-700"
                disabled={isUploading}
              />
              {isUploading && (
                <div className="text-sm text-yellow-400">アップロード中...</div>
              )}
            </div>
            ...
            <audio
              ...
              onLoadedMetadata={handleSyncDuration}
            />
            {audioError && (
              <p className="text-sm text-red-500">音源読み込みエラー</p>
            )}
            ...
            <button
              onClick={saveChart}
              className="w-full px-4 py-3 bg-yellow-500 hover:bg-yellow-600 rounded text-gray-900 font-bold"
            >
