...
export default function Result() {
  ...
  if (state && state.isMultiPlayer) {
    ...
    if (score > opponentScore) {
      result = 'WIN'; color = "text-green-400";
    } else if (score < opponentScore) {
      result = 'LOSE'; color = "text-red-400";
    } else {
      result = 'DRAW'; color = "text-yellow-400";
    }
  ...
  if (state && state.counts1 && state.counts2) {
    ...
    const accuracy1 = total1 > 0 ? Math.round(((c1.perfect ?? 0) + (c1.good ?? 0) * 0.5) / total1 * 100) : 0;
    const total2 = (c2.perfect ?? 0) + (c2.good ?? 0) + (c2.miss ?? 0);
    const accuracy2 = total2 > 0 ? Math.round(((c2.perfect ?? 0) + (c2.good ?? 0) * 0.5) / total2 * 100) : 0;
    
    let result1 = "", result2 = "", color1 = "", color2 = "";
    if (s1 > s2) {
        result1 = "WIN"; color1 = "text-green-400";
        result2 = "LOSE"; color2 = "text-red-400";
    } else if (s1 < s2) {
        result1 = "LOSE"; color1 = "text-red-400";
        result2 = "WIN"; color2 = "text-green-400";
    } else {
        result1 = "DRAW"; color1 = "text-yellow-400";
        result2 = "DRAW"; color2 = "text-yellow-400";
    }
      ...
      <div className="h-screen flex flex-col items-center justify-center bg-slate-900 text-white">
        <div className={`flex flex-col items-center gap-8 transition-all duration-1000 ${
          ...
          <div className="flex flex-row items-start justify-center gap-16">
            {/* Player 1 Result */}
            <div className="flex flex-col items-center gap-4">
              <h2 className={`text-4xl font-bold ${color1}`}>1P <span className="ml-2">{result1}</span></h2>
              <div className="text-center">
                <div className="text-6xl font-bold text-yellow-400">{s1.toLocaleString()}</div>
                <div className="text-lg text-gray-300">SCORE</div>
              </div>
              <div className="flex gap-6">
                <div className="text-center">
                  <div className="text-2xl font-bold text-green-400">{c1.perfect ?? 0}</div>
                  <div className="text-sm text-gray-400">Perfect</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-blue-400">{c1.good ?? 0}</div>
                  <div className="text-sm text-gray-400">Good</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-red-400">{c1.miss ?? 0}</div>
                  <div className="text-sm text-gray-400">Miss</div>
                </div>
              </div>
              <div className="text-center">
                <div className="text-xl font-bold text-purple-400">{accuracy1}%</div>
                <div className="text-sm text-gray-400">Accuracy</div>
              </div>
            </div>
            ...
            {/* Player 2 Result */}
            <div className="flex flex-col items-center gap-4">
              <h2 className={`text-4xl font-bold ${color2}`}>2P <span className="ml-2">{result2}</span></h2>
              <div className="text-center">
                <div className="text-6xl font-bold text-yellow-400">{s2.toLocaleString()}</div>
                <div className="text-lg text-gray-300">SCORE</div>
              </div>
              <div className="flex gap-6">
                <div className="text-center">
                  <div className="text-2xl font-bold text-green-400">{c2.perfect ?? 0}</div>
                  <div className="text-sm text-gray-400">Perfect</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-blue-400">{c2.good ?? 0}</div>
                  <div className="text-sm text-gray-400">Good</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-red-400">{c2.miss ?? 0}</div>
                  <div className="text-sm text-gray-400">Miss</div>
                </div>
              </div>
              <div className="text-center">
